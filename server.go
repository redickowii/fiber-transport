...
func (s Server) Handler(c *fiber.Ctx) error {
	...
	if len(s.finalizer) > 0 {
		defer func(c *fiber.Ctx) {
			ctx = context.WithValue(ctx, ContextKeyResponseHeaders, c.GetRespHeaders())
			for _, f := range s.finalizer {
				f(ctx, c.Context().Response.StatusCode(), c)
			}
		}(c)
	}
...
type ErrorEncoder func(ctx context.Context, err error, c *fiber.Ctx)
...
type ServerFinalizerFunc func(ctx context.Context, code int, c *fiber.Ctx)
...
func NopRequestDecoder(ctx context.Context, c *fiber.Ctx) (interface{}, error) {
	return nil, nil
}
...
func EncodeJSONResponse(_ context.Context, c *fiber.Ctx, response interface{}) error {
...
func DefaultErrorEncoder(_ context.Context, err error, ctx *fiber.Ctx) {
